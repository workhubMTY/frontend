...
import { Search, UserPlus, Clock, MapPin, ChevronLeft, ChevronRight, Star, Calendar } from "lucide-react";
...
function EventoGeneralDetail({
  evento, onPrev, onNext, dotCount, dotActive, onDot,
}: {
  ...
  return (
    <div className="shrink-0 rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
      <div className="flex items-center justify-between border-b border-gray-100 px-4 py-2.5">
        <h3 className="text-[0.85rem] font-semibold text-gray-900">Eventos & Festivos</h3>
      ...
      <div className="flex gap-3 px-4 py-3">
        ...
        <div className="flex flex-1 flex-col gap-1">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-[13px] font-semibold text-gray-900">{evento.titulo}</p>
...
export default function Home() {
  ...
  const handlePersonClick = (i: number) => setSelectedPerson(selectedPerson === i ? null : i);
      ...
      <section className="flex h-[100svh] w-full flex-col bg-background-page overflow-hidden">
        <div className="flex h-full w-full flex-col px-[3%] py-[2%]">
          ...
          >
              ...
              <div className="flex flex-1 flex-col gap-1 overflow-y-auto min-h-0">
                {PERSONAS.map((p, i) => (
                  <button key={i} onClick={() => handlePersonClick(i)}
                    ...
                    style={{ background: selectedPerson === i ? "#FFF0E6" : "transparent" }}>
                    ...
                    {selectedPerson === i && (
                      <Star size={10} className="shrink-0" style={{ color: "#F97316" }} />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden p-4 gap-3">
              <div className="flex shrink-0 items-center justify-between">
                <span className="text-[0.85rem] font-semibold text-gray-900">Invitaciones</span>
              ...
              {selInv !== null && (
                <div className="flex items-center gap-1.5 rounded-lg bg-blue-50 border border-blue-100 px-2 py-1.5">
                  ...
                  <p className="text-[10px] text-blue-700 leading-tight">Invitación marcada en la agenda</p>
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto min-h-0">
                {INVITACIONES.map((sec, si) => (
                  <div key={si}>
                    <p className="py-1 text-[9px] font-bold uppercase tracking-wider text-gray-400">{sec.dia}</p>
                    {sec.items.map((item, ii) => {
                      ...
                      return (
                        <div key={ii}
                          ...
                          style={{ background: isSelected ? "#E6F4FF" : "transparent", borderColor: isSelected ? "#93C5FD" : "transparent" }}
